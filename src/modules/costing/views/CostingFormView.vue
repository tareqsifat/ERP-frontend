...
// PRD v1 §3.3 "Costing Form". total_cost is displayed as a live
// client-side estimate only — the API always recomputes it server-side
// (backend Modules/Costing/README.md).
...
const estimatedTotal = computed(
  () => (Number(form.costed_quantity) || 0) * (Number(form.average_unit_cost) || 0),
)
...
onMounted(async () => {
  ...
  if (isEdit.value) {
    const { data: costingData } = await costingApi.get(route.params.id)
    Object.assign(form, {
      order_id: costingData.data.order_id,
      style: costingData.data.style,
      costed_quantity: costingData.data.costed_quantity,
      average_unit_cost: costingData.data.average_unit_cost,
      status: costingData.data.status,
    })
  }
})
...
async function handleSubmit() {
  ...
  try {
    if (isEdit.value) {
      await store.updateCosting(route.params.id, form)
    } else {
      await store.createCosting(form)
    }
    router.push({ name: 'costings.index' })
  } catch (error) {
    errorMessage.value = error.response?.data?.message || 'Could not save this costing.'
  } finally {
...
<template>
  <div class="mx-auto max-w-lg p-6">
    <h1 class="mb-4 text-lg font-semibold text-gray-900">
      {{ isEdit ? 'Edit Costing' : 'Add New Costing' }}
    </h1>
    ...
    <form class="space-y-4" @submit.prevent="handleSubmit">
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          <label class="mb-1 block text-sm font-medium text-gray-700">Costed Quantity</label>
          <input v-model.number="form.costed_quantity" type="number" min="1" required class="w-full rounded border border-gray-300 px-3 py-2 text-sm" />
        </div>
        <div>
          <label class="mb-1 block text-sm font-medium text-gray-700">Average Unit Cost</label>
          <input v-model.number="form.average_unit_cost" type="number" step="0.01" min="0" required class="w-full rounded border border-gray-300 px-3 py-2 text-sm" />
        </div>
      </div>
